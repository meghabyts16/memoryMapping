...
int writePage(struct file *f, char *addr, int size, int offset) {
    int n;
    // Write the data from memory (addr) to the file (f) at the specified offset
    if ((n = filewrite(f, addr, size)) < 0) {
